...
#include "proxySerial1.h"
#include "Serial_LCD1.h"


// Constructor
Serial_LCD1::Serial_LCD1(ProxySerial1 * port0) {
  _port=port0;
}
...
uint8_t Serial_LCD1::setSpeed(uint16_t speed) {
  uint8_t a=0x06;
...
uint8_t Serial_LCD1::clear() {
  _port->print('E');
  return nacAck();
}

void Serial_LCD1::off() {
  clear();
...
uint8_t Serial_LCD1::setBacklight(bool b) {
  _port->print('Y');
  _port->print((char)0x00);
...
uint8_t Serial_LCD1::setDisplay(bool b) {
  _port->print('Y');
...
uint8_t Serial_LCD1::setContrast(uint8_t b) {
  if (b<=0x0f) {
    _port->print('Y');
...
uint8_t Serial_LCD1::setOrientation(uint8_t b) {   // Display Control Functions – 59hex
  _orientation=b;
  _port->print('Y');
...
uint8_t Serial_LCD1::getOrientation() { 
  return _orientation; 
} 

uint8_t Serial_LCD1::setTouch(bool b) {
  if (b) {
    _port->print('Y');
...
uint8_t Serial_LCD1::setVolume(uint8_t percent) { // Set Volume - 76hex 
...
// Graphics
uint8_t Serial_LCD1::circle(uint16_t x1, uint16_t y1, uint16_t radius, uint16_t colour) {
  _port->print('C');
...
uint8_t Serial_LCD1::rectangle(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t colour) {
  _port->print('r');
...
uint8_t Serial_LCD1::ellipse(uint16_t x, uint16_t y, uint16_t rx, uint16_t ry, uint16_t colour) {
  _port->print('e');
...
uint8_t Serial_LCD1::line(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t colour) {
  _port->print('L');
...
uint8_t Serial_LCD1::setBackGroundColour(uint16_t colour) { 
  _port->print('K');
...
uint8_t Serial_LCD1::point(uint16_t x1, uint16_t y1, uint16_t colour) {
  _port->print('P');
...
uint16_t Serial_LCD1::readPixel(uint16_t x1, uint16_t y1) {
  _port->print('R');
...
uint8_t Serial_LCD1::triangle(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t x3, uint16_t y3, uint16_t colour) {
  bool b=true;
...
uint8_t Serial_LCD1::setPenSolid(bool b) {
...
uint8_t Serial_LCD1::setFont(uint8_t b) {
...
uint8_t Serial_LCD1::setFontSolid(uint8_t b) {
...
uint8_t Serial_LCD1::tText(uint8_t x, uint8_t y, uint16_t colour, String s) {
  _port->print('s');
...
uint8_t Serial_LCD1::gText(uint16_t x, uint16_t y, uint16_t colour, String s) {
  _port->print('S');
...
//     2 : Touch Release 
//     3 : Touch Moving
uint8_t Serial_LCD1::getTouchActivity() {
  _port->print('o');
  _port->print((char)0x04);   // state
...
uint8_t Serial_LCD1::getTouchXY(uint16_t &x, uint16_t &y) {
  _port->print('o');
...
uint8_t Serial_LCD1::detectTouchRegion(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2) {
  _port->print('u');
...
// default option = 0
uint8_t Serial_LCD1::writeString2File(String filename, String text, uint8_t option) { 
  String s;
  uint8_t a;
  uint8_t j;
...
uint8_t Serial_LCD1::appendString2File(String filename, String text) { 
  return writeString2File(filename, text, 0x80);  // append option
}

// Erase file from Card (FAT) - @65hex 
uint8_t Serial_LCD1::eraseFile(String filename) {  
  if (_checkedSD==1) {
    _port->print('@');
    _port->print('e');
    _port->print(filename);
...
uint8_t Serial_LCD1::findFile(String filename) {  
  if (_checkedSD==1) {
    _port->print('@');
    _port->print('d');
    _port->print(filename);
...
uint8_t Serial_LCD1::saveScreenSD(String filename) {    
  saveScreenSD(filename, 0, 0, 319, 239);
}

// Screen Copy-Save to Card (FAT) - @63hex 
// x1, y1 x2, y2: same coordinates as rectangle
uint8_t Serial_LCD1::saveScreenSD(String filename, uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2) {    
  if (_checkedSD==1) {
    _port->print('@');
    _port->print('c');
...
uint8_t Serial_LCD1::readScreenSD(String filename) {   
  readScreenSD(filename, 0, 0);
}

// Display Image-Icon from Card (FAT) - @6Dhex 
// x1, y1: left-top coordinates
uint8_t Serial_LCD1::readScreenSD(String filename, uint16_t x1, uint16_t y1) {   
  if (_checkedSD==1) {
    _port->print('@');
    _port->print('m');
    _port->print(filename);
...
uint8_t Serial_LCD1::playSoundSD(String filename, uint8_t option0) {   
  if (_checkedSD==1) {
...
uint16_t Serial_LCD1::rgb16(uint8_t red8, uint8_t green8, uint8_t blue8) {
  // deprecated
  return (red8 >> 3) << 11 | (green8 >> 2) << 5 | (blue8 >> 3);
}


uint16_t Serial_LCD1::setColour(uint8_t red8, uint8_t green8, uint8_t blue8) {
    // rgb16 = red5 green6 blue5
    return (red8 >> 3) << 11 | (green8 >> 2) << 5 | (blue8 >> 3);
}


void Serial_LCD1::splitColour(uint16_t rgb, uint8_t &red, uint8_t &green, uint8_t &blue) {
  // rgb16 = red5 green6 blue5
...
uint16_t Serial_LCD1::halfColour(uint16_t rgb) {
    // rgb16 = red5 green6 blue5 
...
void Serial_LCD1::_swap(uint16_t &a, uint16_t &b) {
  uint16_t w=a;
  a=b;
  b=w;
}
