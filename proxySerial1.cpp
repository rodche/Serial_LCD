...
ProxySerial1::ProxySerial1(Stream * port0) {
  _proxyPort = port0; 
}


void ProxySerial1::_checkSpeed() {  
  //    while(!(millis()-_millis > 3));    // time in ms
  while((millis()-_millis < securityDelay));    // time in ms, the same !> = <
  _millis=millis();
}

void ProxySerial1::begin(uint16_t b) {  // to be managed at serial port level
  _checkSpeed();  
//  _proxyPort->begin(b); 
}

void ProxySerial1::print(int8_t i) { 
  _checkSpeed();  
  _proxyPort->write(i); 
}

void ProxySerial1::print(uint8_t ui) { 
  _checkSpeed();  
  _proxyPort->write(ui); 
};

void ProxySerial1::print(int16_t i) { 
  _checkSpeed();  
  _proxyPort->write(highByte(i)); 
  _proxyPort->write(lowByte(i)); 
};

void ProxySerial1::print(uint16_t ui) { 
  _checkSpeed();  
  _proxyPort->write(highByte(ui)); 
  _proxyPort->write(lowByte(ui)); 
};

void ProxySerial1::print(char c) { 
  _proxyPort->write((uint8_t)c); 
};
void ProxySerial1::print(String s) { 
...
uint8_t ProxySerial1::read() { 
  _checkSpeed();  
  return _proxyPort->read(); 
}
int8_t ProxySerial1::available() { 
  _checkSpeed();  
  return _proxyPort->available(); 
}
void ProxySerial1::flush() {  
  _checkSpeed();  
  _proxyPort->flush(); 
}
