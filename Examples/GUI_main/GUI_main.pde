...
//   Serial.print(sizeRAW, DEC);
   Serial.print("\n");
   for (int i=0; i<8; i++)       Serial.print(myLCD.dSaveScreenRAW( sizeRAW*(i+0),  0, i*30, 80, 30 ), HEX); 
   
   myLCD.line(20, 0, 20, 239, 0xffff);
   delay(500);
   
   Serial.print("\n");
   for (int i=0; i<8; i++)       Serial.print(myLCD.readScreenRAW( sizeRAW*(i+0),  0, i*30 ), HEX); 
   Serial.print("\n");
...
void loop() {
  ...
  if (b) {
    myButton.dStringDefine(&myLCD, 160, 120, 79, 59, String(result, HEX), myLCD.rgb16(0xff, 0xff, 0xff), myLCD.rgb16(0xff, 0x00, 0x00), myLCD.rgb16(0x88, 0x00, 0x00), 9);
  ...
  if (c>0) {
    ...
    if (myButton.check()) {
      String s;
      if ( 1 == dialog(&myLCD, "Quit?", 3, myLCD.setColour(0xff,0xff,0xff), myLCD.setColour(0x20,0x20,0x20), myLCD.setColour(0x80,0x80,0x80), String("Are you sure to quit?"), "Yes", myLCD.setColour(0xff,0xff,0xff), myLCD.setColour(0x00,0xff,0x00), myLCD.rgb16(0x00,0x80,0x00), "Please confirm.","No",myLCD.rgb16(0xff,0xff,0xff), myLCD.rgb16(0xff,0x00,0x00), myLCD.rgb16(0x80,0x00,0x00)) ) {
        delay(2000);
        myLCD.off();
        while(true);
      } 
